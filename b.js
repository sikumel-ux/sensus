...
// Karena form onsubmit dimatikan, kita validasi manual secara ketat lewat JS
function prosesKolektifKirimData() {
    ...
    .then(() => {
        tampilkanHalamanTerimaKasih(kodeRegistrasi, payloadSensus);
    })
    .catch(err => {
...
function tampilkanHalamanTerimaKasih(kode, data) {
    document.getElementById("registrasiKodeContainer").innerHTML = `
        <div class="code-display">${kode}</div>
    `;

    let htmlSummary = `
        <div class="form-group"><label>Status Rumah</label><div class="review-value">${data.statusRumah}</div></div>
        ${data.statusRumah !== "Kos" ? `<div class="form-group"><label>No. KK</label><div class="review-value">${data.noKk}</div></div>` : ''}
        <div class="form-group"><label>No Rumah & Kontak</label><div class="review-value">No. ${data.noRumah} / WA Utama: ${data.waUtama}</div></div>
        <div class="form-group"><label>Aturan Kewajiban</label><div class="review-value" style="color:var(--primary); font-weight:700;">${data.statusRumah === "Kos" ? "Bebas Jimpitan (-)" : "Wajib Jimpitan (" + data.modeJimpitan + ")"}</div></div>
        ${data.statusRumah === "Kos" ? `<div class="form-group"><label>Nama Pemilik/Pengelola Kos</label><div class="review-value">${data.namaPemilikKos}</div></div>` : ''}
        <div class="form-group"><label>Alamat Lokasi</label><div class="review-value">${data.alamat}</div></div>
        <div style="margin-top:5px;"><b style="font-size:0.75rem; color:var(--text-muted)">DAFTAR ANGGOTA JIWA (${data.anggota.length} Orang):</b></div>
    `;

    data.anggota.forEach((w, idx) => {
        htmlSummary += `
            <div class="warga-item-box" style="border-style: solid; background: #fff; padding: 12px; margin-bottom: 8px;">
                <div style="font-weight:700; color:var(--primary)">#${idx + 1} ${w.nama} <span class="review-badge">${w.hubungan}</span></div>
                <p style="font-size:0.75rem; color:var(--text-muted); margin-top:2px;">NIK: ${w.nik} | TTL: ${w.ttl} | Pekerjaan: ${w.pekerjaan}</p>
            </div>
        `;
    });

    document.getElementById("reviewDataTerimaKasih").innerHTML = htmlSummary;

    const tkCard = document.querySelector("#pageTerimaKasih .app-card");
    if(tkCard) tkCard.style.borderTop = "4px solid var(--primary)";

    document.getElementById("pageFormulir").style.display = "none";
    document.getElementById("pageTerimaKasih").style.display = "block";
    window.scrollTo({ top: 0, behavior: "smooth" });
}

function resetKembaliKeFormAwal() {
    const btn = document.getElementById("btnSubmitFinal");
    netralkanTombolSubmit(btn);

    // Reset Form & bersihkan sisa row dinamis
    document.getElementById("mainSensusForm").reset();
    document.getElementById("wargaListContainer").innerHTML = "";
    rowCount = 0;
    
    // Inisialisasi ulang form segar
    sinkronisasiKategoriWargaAll();
    tambahWargaRow();

    document.getElementById("pageTerimaKasih").style.display = "none";
    document.getElementById("pageFormulir").style.display = "block";
    window.scrollTo({ top: 0, behavior: "smooth" });
}
